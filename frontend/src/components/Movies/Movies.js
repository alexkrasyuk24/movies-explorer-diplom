...
function Movies({ loggedIn, handleLikeFilm, onDeleteCard, savedMovies }) {
  ...
  function handleMovieFiltering(movies, query, short) {
    ...
    setFilteredMovies(
      short ? filterDurationFilms(moviesCardList) : moviesCardList
    )
    localStorage.setItem("movies", JSON.stringify(moviesCardList))
    localStorage.setItem("allMovies", JSON.stringify(movies))
  }

  // Функция setSearchTerm используется для поиска фильмов
  ...
  function toggleShortFilmVisibility() {
    ...
    if (!isShortMovies) {
      if (filterDurationFilms(initialCardsMovies).length === 0) {
        setFilteredMovies(filterDurationFilms(initialCardsMovies))
      } else {
        setFilteredMovies(filterDurationFilms(initialCardsMovies))
      }
    } else {
  ...
  useEffect(() => {
    if (localStorage.getItem("movies")) {
      ...
      if (localStorage.getItem("shortMovies") === "true") {
        setFilteredMovies(filterDurationFilms(movies))
      } else {
